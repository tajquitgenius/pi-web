...
// (a single self-contained IIFE that compiles the retained export-only Svelte graph).
...
// the same constraint as the desktop and mobile React build outputs.
//
...
func RenderExportSessionPage(session sessions.Session, theme string) string {
	dataBase64, css, bodyAttrs := prepareSessionPageData(session, exportThemeCSS+"\n"+exportSessionCSS)
