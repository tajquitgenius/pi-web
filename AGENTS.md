...
- **Always `make build`, never `go build` alone** — `//go:embed` needs `web/dist-desktop`, `web/dist-mobile`, and `internal/ui/embedded/export/export.js` from the frontend build.

## Critical Rules

1. **Live products and export are separate renders.** Live = independent React desktop/mobile products via `internal/ui/embedded/app.html` (`spa_page.go`). Export/share = a read-only static snapshot via `internal/ui/embedded/share-session.html` (`export.go`), built from the isolated Svelte graph rooted at `web/src/export/export-entry.js`. Never import live transport or chrome into export.
...
4. **Icons:** Lucide only. React uses `lucide-react`; static export uses `web/src/shared/icons.js`. No hand-drawn SVG or unicode glyphs.
